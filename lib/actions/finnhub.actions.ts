...
import { formatArticle, formatChangePercent, formatMarketCapValue, formatPrice, getDateRange, validateArticle } from '@/lib/utils';
...
export async function getStockOverview(symbol: string): Promise<StockOverview | null> {
    try {
        ...
        const [quote, profile, financials] = await Promise.all([
            fetchJSON<QuoteData>(`${FINNHUB_BASE_URL}/quote?symbol=${encodeURIComponent(cleanSymbol)}&token=${token}`, 60),
            fetchJSON<FinnhubProfile>(`${FINNHUB_BASE_URL}/stock/profile2?symbol=${encodeURIComponent(cleanSymbol)}&token=${token}`, 3600),
            fetchJSON<FinancialsData>(`${FINNHUB_BASE_URL}/stock/metric?symbol=${encodeURIComponent(cleanSymbol)}&metric=all&token=${token}`, 3600),
        ]);
        ...
        return {
            ...
            industry: profile.finnhubIndustry || 'Market',
            ...
            changeFormatted: formatChangePercent(quote.dp) || 'N/A',
